...
@interface UIImage (RedGramClassicIOS6Resources)

@end
@implementation UIImage (RedGramClassicIOS6Resources)
...
+ (void)load
{
    ...
    dispatch_once(&onceToken, ^
    {
        ...
        Method replacement = class_getClassMethod(self, @selector(RedGram_classicIOS6ImageNamed:));
        method_exchangeImplementations(original, replacement);
    });
}

+ (UIImage *)RedGram_classicIOS6ImageNamed:(NSString *)name
{
    UIImage *classicImage = TGClassicIOS6OriginalImageNamed(name);
    return classicImage != nil ? classicImage : [self RedGram_classicIOS6ImageNamed:name];
}
...
static void TGIOS6SendLatestSystemCrashReport()
{
    ...
    if (report.length == 0)
        return;
    if ([report rangeOfString:@"com.example.RedGram"].location == NSNotFound)
        return;
...
- (TGNavigationController *)loginNavigationController
{
    if (_loginNavigationController == nil)
    {
        ...
        if ([bundleIdentifier isEqualToString:@"com.example.RedGram"]) {
...
{
    if ([userActivityType isEqualToString:@"com.example.RedGram.conversation"]) {
...
- (BOOL)application:(UIApplication *)__unused application continueUserActivity:(NSUserActivity *)userActivity restorationHandler:(void (^)(NSArray *))__unused restorationHandler
{
    ...
    } else if ([userActivity.activityType isEqualToString:@"com.example.RedGram.conversation"]) {
