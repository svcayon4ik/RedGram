...
static bool TGRedGramDarkLoginAppearance(void)
{
    TGPresentation *presentation = [TGPresentation current];
    return [TGPresentation classicIOS6Style] || (presentation != nil && presentation.pallete.isDark);
}

static UIImage *TGRedGramLoginSurfaceImage(UIColor *color, UIColor *borderColor)
{
    CGSize size = CGSizeMake(40.0f, 56.0f);
    UIGraphicsBeginImageContextWithOptions(size, false, 0.0f);
    CGRect rect = CGRectInset((CGRect){CGPointZero, size}, 0.5f, 0.5f);
    ...
    return [image stretchableImageWithLeftCapWidth:19 topCapHeight:27];
}
...
- (id)init
{
    ...
    if (self)
    {
        ...
        if (TGRedGramDarkLoginAppearance())
...
- (void)loadView
{
    ...
    bool darkAppearance = TGRedGramDarkLoginAppearance();
    UIColor *textColor = darkAppearance ? UIColorRGB(0xf0f0f0) : [UIColor blackColor];
    UIColor *placeholderColor = darkAppearance ? UIColorRGB(0x858585) : UIColorRGB(0xc7c7cd);
    UIColor *surfaceColor = darkAppearance ? UIColorRGB(0x202020) : [UIColor whiteColor];
    UIColor *surfaceBorderColor = darkAppearance ? UIColorRGB(0x4a4a4a) : UIColorRGB(0xd0d0d0);

    self.view.backgroundColor = darkAppearance ? UIColorRGB(0x0d0d0d) : [UIColor whiteColor];
    ...
    if (darkAppearance)
    {
        UIImage *surfaceImage = TGRedGramLoginSurfaceImage(surfaceColor, surfaceBorderColor);
        [_countryButton setBackgroundImage:surfaceImage forState:UIControlStateNormal];
        [_countryButton setBackgroundImage:surfaceImage forState:UIControlStateHighlighted];
    }
    ...
    [_countryButton setTitleColor:textColor forState:UIControlStateNormal];
    ...
    if (darkAppearance)
        _inputBackgroundView.image = TGRedGramLoginSurfaceImage(surfaceColor, surfaceBorderColor);
    ...
    _countryCodeField.textColor = textColor;
    ...
    _phoneField.textColor = textColor;
    _phoneField.placeholder = TGLocalized(@"Login.PhonePlaceholder");
    _phoneField.keyboardType = UIKeyboardTypeNumberPad;
    _phoneField.delegate = self;
    _phoneField.placeholderColor = placeholderColor;
