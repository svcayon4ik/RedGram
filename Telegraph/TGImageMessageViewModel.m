...
- (void)layoutForContainerSize:(CGSize)containerSize
{
    ...
    // ========== КРАСНЫЙ ФОН ДЛЯ БАБЛОВ В iOS 6 ТЕМЕ ==========
    if (classicIOS6Style) {
        // Создаём или обновляем фон с красным цветом
        if (_backgroundModel != nil) {
            // Меняем цвет фона у существующей модели
            // В TGTextMessageBackgroundViewModel есть свойство backgroundColor
            // Но проще пересоздать модель с красным цветом
            [self removeSubmodel:_backgroundModel viewStorage:nil];
