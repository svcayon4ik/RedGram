    ...
    dispatch_once(&onceToken, ^
                  {
                      ...
                      // КРАСНЫЙ ГРАДИЕНТ
                      NSArray *colors = @[(id)UIColorRGB(0xcc3333).CGColor, (id)UIColorRGB(0xaa2222).CGColor, (id)UIColorRGB(0x881111).CGColor];
                      ...
                      CGContextSetFillColorWithColor(context, UIColorRGBA(0xff8888, 0.3f).CGColor);
                      CGContextFillRect(context, CGRectMake(0.0f, 0.0f, size.width, 1.0f));
                      
                      // ТЁМНАЯ ЛИНИЯ СНИЗУ
                      CGContextSetFillColorWithColor(context, UIColorRGB(0x660000).CGColor);
                      CGContextFillRect(context, CGRectMake(0.0f, size.height - 1.0f, size.width, 1.0f));
                      
                      // СВЕТЛАЯ ТЕНЬ НАД ТЁМНОЙ ЛИНИЕЙ
                      CGContextSetFillColorWithColor(context, UIColorRGBA(0xff9999, 0.15f).CGColor);
...
- (instancetype)init
{
    ...
    if (self != nil)
    {
        ...
        _color = UIColorRGB(0xcc3333); // КРАСНЫЙ
    }
    return self;
}
...
- (instancetype)initWithMessageCount:(NSUInteger)messageCount
{
    ...
    if (self != nil)
    {
        ...
        _titleLabel.textColor = UIColorRGB(0xcc3333); // КРАСНЫЙ
        ...
        _countBackgroundView.backgroundColor = UIColorRGB(0xcc3333); // КРАСНЫЙ
        ...
        _separatorView.backgroundColor = UIColorRGB(0xcc6666); // КРАСНЫЙ СВЕТЛЫЙ
...
- (instancetype)initWithTitle:(NSString *)title index:(NSUInteger)index action:(void (^)(void))action
{
    ...
    if (self != nil)
    {
        ...
        _indexBackgroundView.backgroundColor = UIColorRGB(0xcc3333); // КРАСНЫЙ
        ...
        _messageLabel.textColor = UIColorRGB(0x333333); // ТЁМНЫЙ (почти чёрный)
...
- (void)setDark
{
    ...
    _separatorView.backgroundColor = UIColorRGB(0x383838);
    _chevronView.color = UIColorRGB(0xcc3333); // КРАСНЫЙ
}

- (void)setPallete:(TGMenuSheetPallete *)pallete
{
    [super setPallete:pallete];
    _messageLabel.textColor = UIColorRGB(0x333333); // ТЁМНЫЙ
    _indexBackgroundView.backgroundColor = UIColorRGB(0xcc3333); // КРАСНЫЙ
    _separatorView.backgroundColor = UIColorRGB(0xcc6666); // КРАСНЫЙ СВЕТЛЫЙ
    _chevronView.color = UIColorRGB(0xcc3333); // КРАСНЫЙ
}
...
- (instancetype)initWithMessage:(TGMessage *)message {
    ...
    if (self != nil) {
        ...
        self.backgroundColor = [TGPresentation classicIOS6Style] ? [UIColor clearColor] : UIColorRGB(0xcc3333); // КРАСНЫЙ
        
        _separatorView = [[UIView alloc] init];
        _separatorView.backgroundColor = [TGPresentation classicIOS6Style] ? UIColorRGB(0x660000) : UIColorRGB(0xcc3333); // КРАСНЫЙ
        [self addSubview:_separatorView];
...
- (void)setPresentation:(TGPresentation *)presentation
{
    ...
    self.backgroundColor = classicIOS6Style ? [UIColor clearColor] : UIColorRGB(0xcc3333); // КРАСНЫЙ
    _separatorView.backgroundColor = classicIOS6Style ? UIColorRGB(0x660000) : UIColorRGB(0xcc3333); // КРАСНЫЙ
