...
static UIImage *TGSearchBarClassicFieldImage(void)
{
    ...
    // Заливка — светлый красный
    CGContextSaveGState(context);
    CGContextSetShadowWithColor(context, CGSizeMake(0.0f, 1.0f), 1.0f, UIColorRGBA(0x000000, 0.32f).CGColor);
    CGContextSetFillColorWithColor(context, UIColorRGB(0xffe6e6).CGColor); // Светло-красный фон
    ...
    CGFloat components[] =
    {
        0.90f, 0.50f, 0.50f, 1.0f,  // красный
        1.00f, 0.75f, 0.75f, 1.0f,  // розовый
        0.95f, 0.60f, 0.60f, 1.0f   // средне-красный
    };
    ...
    // Обводка — тёмно-красная
    CGContextSetLineWidth(context, 1.0f);
    CGContextSetStrokeColorWithColor(context, UIColorRGB(0x8b0000).CGColor); // Тёмно-красный
    CGContextAddPath(context, path.CGPath);
    CGContextStrokePath(context);
    CGContextSetStrokeColorWithColor(context, UIColorRGBA(0xff0000, 0.72f).CGColor);
...
static UIImage *TGSearchBarClassicBackgroundImage(void)
{
    ...
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    // Градиент фона — от тёмно-красного к красному
    CGFloat components[] =
    {
        0.60f, 0.10f, 0.10f, 1.0f,  // тёмно-красный
        0.80f, 0.20f, 0.20f, 1.0f   // красный
    };
    ...
    CGContextSetFillColorWithColor(context, UIColorRGB(0x8b0000).CGColor); // Тёмно-красная линия внизу
...
- (instancetype)initWithFrame:(CGRect)frame style:(TGSearchBarStyle)style
{
    ...
    if (self != nil)
    {
        ...
        // ========== ЧЁРНЫЙ ТЕКСТ ПЛЕЙСХОЛДЕРА ==========
        UIColor *placeholderColor = UIColorRGB(0x000000); // ЧЁРНЫЙ
        ...
        {
            iconImage = [TGSearchBar searchBarIcon:UIColorRGB(0xcc0000)]; // КРАСНЫЙ
        }
...
- (void)setPallete:(TGSearchBarPallete *)pallete
{
    ...
    // ========== ЧЁРНЫЙ ТЕКСТ ==========
    _customTextField.textColor = UIColorRGB(0x000000); // ЧЁРНЫЙ
    _prefixLabel.textColor = UIColorRGB(0x000000);
    _placeholderLabel.textColor = UIColorRGB(0x000000);
    ...
    _customSearchIcon.image = [TGSearchBar searchBarIcon:UIColorRGB(0xcc0000)]; // КРАСНЫЙ
    ...
    if (_style == TGSearchBarStyleLight || _style == TGSearchBarStyleLightPlain || _style == TGSearchBarStyleLightAlwaysPlain || _style == TGSearchBarStyleHeader || _style == TGSearchBarStyleKeyboard)
    {
        ...
        [(TGModernButton *)_customCancelButton setTitleColor:UIColorRGB(0x949494)]; // КРАСНЫЙ
...
- (UIImage *)normalTextFieldBackgroundImage
{
    ...
    if (_highContrast) {
        UIColor *highContrastColor = _pallete != nil ? _pallete.highContrastBackgroundColor : UIColorRGB(0xffe6e6); // Светло-красный
        ...
        {
            UIColor *color = _pallete != nil ? _pallete.backgroundColor : UIColorRGB(0xffe6e6); // Светло-красный
        ...
        else if (_style == TGSearchBarStyleLightAlwaysPlain)
        {
            ...
            UIGraphicsBeginImageContextWithOptions(CGSizeMake(diameter, diameter), false, 0.0f);
            CGContextRef context = UIGraphicsGetCurrentContext();
            CGContextSetFillColorWithColor(context, UIColorRGB(0xffcccc).CGColor); // Светло-красный
            CGContextFillEllipseInRect(context, CGRectMake(0.0f, 0.0f, diameter, diameter));
            UIImage *image = [UIGraphicsGetImageFromCurrentImageContext() stretchableImageWithLeftCapWidth:(NSInteger)(diameter / 2.0f) topCapHeight:(NSInteger)(diameter / 2.0f)];
            UIGraphicsEndImageContext();
        ...
        else if (_style == TGSearchBarStyleKeyboard)
        {
            ...
            UIGraphicsBeginImageContextWithOptions(CGSizeMake(diameter, diameter), false, 0.0f);
            CGContextRef context = UIGraphicsGetCurrentContext();
            CGContextSetFillColorWithColor(context, UIColorRGB(0xffcccc).CGColor);
            CGContextFillEllipseInRect(context, CGRectMake(0.0f, 0.0f, diameter, diameter));
            UIImage *image = [UIGraphicsGetImageFromCurrentImageContext() stretchableImageWithLeftCapWidth:(NSInteger)(diameter / 2.0f) topCapHeight:(NSInteger)(diameter / 2.0f)];
            UIGraphicsEndImageContext();
...
- (UIImage *)activeTextFieldBackgroundImage
{
    ...
    if (_activeTextFieldBackgroundImage == nil)
    {
        ...
        {
            UIColor *color = _pallete != nil ? _pallete.backgroundColor : UIColorRGB(0xffcccc);
            UIGraphicsBeginImageContextWithOptions(CGSizeMake(14.0f, 14.0f), false, 0.0f);
            CGContextRef context = UIGraphicsGetCurrentContext();
            CGContextSetFillColorWithColor(context, color.CGColor);
            CGContextFillEllipseInRect(context, CGRectMake(0.0f, 0.0f, 14.0f, 14.0f));
...
- (UITextField *)customTextField
{
    if (_customTextField == nil)
    {
        ...
        // ========== ЧЁРНЫЙ ТЕКСТ ==========
        UIColor *textColor = UIColorRGB(0x000000);
        UIImage *clearImage = TGImageNamed(@"SearchBarClearIcon.png");
...
- (UIButton *)customCancelButton
{
    if (_customCancelButton == nil)
    {
        ...
        // ========== КРАСНАЯ КНОПКА ОТМЕНЫ ==========
        UIColor *buttonColor = UIColorRGB(0xcc0000);
...
- (UIView *)customScopeButtonContainer
{
    if (_customScopeButtonContainer == nil)
    {
        ...
        [_customSegmentedControl setTitleTextAttributes:@{UITextAttributeTextColor:_pallete != nil ? _pallete.accentColor : UIColorRGB(0xcc0000), UITextAttributeTextShadowColor: [UIColor clearColor], UITextAttributeFont: TGSystemFontOfSize(13)} forState:UIControlStateNormal];
